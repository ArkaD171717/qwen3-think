...
class TestThinkingSession:
    ...
    def test_chat_complexity_override_in_auto_route(self):
        ...
        # "What is 2+2?" would normally be SIMPLE -> NO_THINK; override to COMPLEX
        s.chat("What is 2+2?", complexity=Complexity.COMPLEX)
        assert len(s.messages) == 2
        # The router still decides the mode (SIMPLE -> NO_THINK), but the
        # complexity override should not crash and the call should succeed.
        # Verify the mode was set by the router (not left uninitialized).
        assert s.thinking_mode == ThinkingMode.NO_THINK
